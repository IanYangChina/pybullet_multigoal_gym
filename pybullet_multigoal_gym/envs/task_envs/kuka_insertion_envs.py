...
from pybullet_multigoal_gym.envs.base_envs.kuka_insertion_base_env import KukaBulletInsertionEnv


class KukaInsertionEnv(KukaBulletInsertionEnv):
    def __init__(self, render=True, binary_reward=True, distance_threshold=0.05,
        ...
        KukaBulletInsertionEnv.__init__(self, render=render, binary_reward=binary_reward,
                                        distance_threshold=distance_threshold,
                                        image_observation=image_observation, goal_image=goal_image,
                                        depth_image=depth_image, pcd=pcd, state_noise=state_noise,
    ...
    def _generate_goal(self):
        (x, y, z), (a, b, c, w), _, _, _, _ = self._p.getLinkState(self.object_bodies['slot'], 3)
        target_obj_quat = np.array([a, b, c, w])
        orientation_euler = quat.as_euler_angles(quat.as_quat_array([w, a, b, c]))

        self.desired_goal = np.concatenate([np.array([x, y, self.object_initial_pos[self.goal_object_key][2]]), orientation_euler], axis=-1)
